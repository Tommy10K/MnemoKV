...
failover, opt-in automatic recovery, workloads, metrics, and a browser learning interface.

It is deliberately not a production Redis replacement. In particular, it does not provide
client authentication, TLS, RESP3, a write-ahead log, data-plane quorum replication, or
production-grade operational hardening. The automatic recovery controller uses Raft only for
control-plane decisions and has an explicit one-failure-at-a-time guarantee.
...
| [`internal/controller`](../../internal/controller/) | Optional Raft-backed automatic recovery controller |
| [`internal/controlplane`](../../internal/controlplane/) | Controller request signing, fencing, and status contracts |
...
- Opt-in five-node automatic recovery with Raft-backed planning, repair, mandatory rebalancing,
  authenticated data-node fencing, and fresh returning-node admission.
- Health, engine, metrics, cluster, controller, command, policy, snapshot, and cluster-admin HTTP
  endpoints.
...
5. [`internal/controller/controller.go`](../../internal/controller/controller.go) for automatic recovery composition.
6. [`frontend/src/app/routes.tsx`](../../frontend/src/app/routes.tsx) for the complete UI map.
