...
005 remains valid for that mode. Automatic mode requires exactly five consistently configured peers,
at least three available Raft voters to commit, separate per-node `controller.raftDir` storage, and a
shared non-empty request-signing secret.
...
promote, replica-assignment, and full-slot-sync operations. Every mutation carries an authenticated,
persisted control index; automatic nodes reject unsigned manual topology mutations and stale or
conflicting indexes.
